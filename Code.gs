  ...
  for (var i = 0; i < linhas.length; i++) {
    ...
    if (!linha.id) {
      try {
        ...
        var detalheNovo = 'linha ' + linha.rowNumber + ' | título: ' + linha.titulo + ' | tipo: ' + linha.tipo + ' | origem: ' + linha.origem + ' | criação: ' + obterMensagemErro_(erroNovo);
        resultado.erros.push(detalheNovo);
        console.error(detalheNovo);
      }
      continue;
    }
    ...
    if (!snapshot[linha.id]) {
      if (linha.tipo === 'Tarefa') {
        try {
          ...
          var detalheTarefaNova = 'linha ' + linha.rowNumber + ' | título: ' + linha.titulo + ' | tipo: ' + linha.tipo + ' | origem: ' + linha.origem + ' | criação de tarefa: ' + obterMensagemErro_(erroTarefaNova);
          resultado.erros.push(detalheTarefaNova);
          console.error(detalheTarefaNova);
        }
      } else if (linha.tipo === 'Evento único' || linha.tipo === 'Evento recorrente') {
        try {
          criarEventoDaLinha_(linha);
          ...
          var detalheEventoNovo = 'linha ' + linha.rowNumber + ' | título: ' + linha.titulo + ' | tipo: ' + linha.tipo + ' | origem: ' + linha.origem + ' | criação de evento: ' + obterMensagemErro_(erroEventoNovo);
          resultado.erros.push(detalheEventoNovo);
          console.error(detalheEventoNovo);
        }
      } else {
        resultado.ignorados++;
      }
    ...
    try {
      if (linha.id.indexOf('TAREFA|') === 0) {
        ...
        var eventoAtualizado = atualizarEventoDaLinha_(linha, anterior);
        if (eventoAtualizado) {
          resultado.atualizados++;
        } else {
          resultado.ignorados++;
      ...
      var detalheAtualizacao = 'linha ' + linha.rowNumber + ' | título: ' + linha.titulo + ' | tipo: ' + linha.tipo + ' | origem: ' + linha.origem + ' | ID: ' + linha.id + ' | atualização: ' + obterMensagemErro_(erro);
      resultado.erros.push(detalheAtualizacao);
      console.error(detalheAtualizacao);
    }
  }
...
function atualizarEventoDaLinha_(linha, anterior) {
  ...
  if (!evento) {
    // O evento pode ter sido apagado ou o ID pode ser antigo.
    // Não interrompe a sincronização nem registra erro repetitivo.
    return false;
  }
  ...
  // O Google Calendar não permite alterar o horário diretamente desta forma
  // em uma ocorrência de evento recorrente. Mantemos o horário original para
  // evitar o erro "Action not allowed". Eventos únicos continuam editáveis.
  if (linha.tipo !== 'Evento recorrente' && (atual.inicio !== antigo.inicio || atual.fim !== antigo.fim)) {
    var inicio = converterParaData_(linha.inicio);
    var fim = converterParaData_(linha.fim);
